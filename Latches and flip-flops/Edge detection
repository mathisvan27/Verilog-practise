...
// a dual edge detection program can also be written by modifying this program (detecting when the input goes to a 1 and goes back to a 0)
...
module top_module (
    ...
    always @(posedge clk) begin
        ...
        pedge <= in & ~prev; // pedge <= (in & ~prev) | (~in & prev) if using dual edge detection
